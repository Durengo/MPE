...
namespace PONG
{
    Game::Game(Player *lplayer, Player *rplayer, Ball *ball)
        : leftPlayer(lplayer), rightPlayer(rplayer), BALL(ball)
    {
        BallStart();
    }

    Game::Game(Player *lplayer, Player *rplayer, Ball *ball, glm::vec4 bounds)
        : leftPlayer(lplayer), rightPlayer(rplayer), BALL(ball), Bounds(bounds)
    {
        BallStart();
    }
    ...
    void Game::OnUpdate(MPE::_TIME deltatime)
    {
        MoveBall(deltatime);
        CheckBallBounds();
    ...
    void Game::MoveBall(MPE::_TIME deltatime)
    {
        if (BallDirectionX == DIRECTION::LEFT)
        {
    ...
    void Game::BallRicochet(MPE::_TIME deltatime)
    {
    }
    ...
    void Game::PlayerMovement(MPE::_TIME deltatime)
    {
        // LEFT PLAYER
    ...
    {
        if (PlayerCollider(leftPlayer, BALL))
        {
            SwitchBallX();
        }
        if (PlayerCollider(rightPlayer, BALL))
        {
            SwitchBallX();
        }
    ...
    bool Game::PlayerCollider(Player *player, Ball *ball)
    {
        bool collisionX = player->GetPosition().x + player->GetSize().x >= ball->GetPosition().x &&
        ball->GetPosition().x + ball->GetSize().x >= player->GetPosition().x;

        bool collisionY = player->GetPosition().y + player->GetSize().y >= ball->GetPosition().y &&
